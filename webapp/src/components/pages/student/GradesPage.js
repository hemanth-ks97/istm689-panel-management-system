...
const Row = ({ row }) => {
  ...
  const renderHelpIcon = (message) => (
    <Tooltip title={<div style={{ lineHeight: '1.2' }}>{message}</div>} arrow>
      <IconButton size="small" sx={{ position: 'relative', top: '-5px', left: '2px', marginLeft: '5px', padding: 0 }}>
  ...
  const renderGradeType = (gradesRow) => (
    <Box sx={{ display: 'flex', alignItems: 'center' }}>
      ...
      {gradesRow.type === 'Submit Questions Stage' && renderHelpIcon("Improve your score by submitting all designated questions for the panel, focusing on the quality of your responses, and maintaining active engagement throughout the stage")}
      {gradesRow.type === 'Tagging Questions Stage' && renderHelpIcon("Improve your score by actively engaging throughout the stage: like, dislike, and group similar questions to demonstrate thoughtful participation")}
      {gradesRow.type === 'Voting Questions Stage' && renderHelpIcon("Improve your score by actively participating in voting throughout the stage")}
    </Box>
  );
  ...
  const SVGProgressBar = ({ grade, mean, max, min }) => {
    ...
    return (
      <Box sx={{ float: 'right', marginRight: '30px', cursor: 'pointer' }}>
        <svg viewBox="0 0 160 30" xmlns="http://www.w3.org/2000/svg" style={{ width: '161px', height: '30px' }}>
          <title>Median: {mean}, High: {max}, Low: {min}</title>
          {/* Base lines */}
          <line x1="0" y1="3" x2="0" y2="27" stroke="#556572" />
          <line x1={svgWidth} y1="3" x2={svgWidth} y2="27" stroke="#556572" />
  
          {/* Statistical Markers */}
          <line x1={minPos} y1="6" x2={minPos} y2="24" stroke="#556572" strokeWidth="2" />
          <line x1={minPos} y1="15" x2={maxPos} y2="15" stroke="#556572" strokeWidth="2" />
          <line x1={maxPos} y1="6" x2={maxPos} y2="24" stroke="#556572" strokeWidth="2" />
          <rect x={meanPos} y="3" width="0" height="24" stroke="#556572" strokeWidth="2" rx="3" fill="none" />
          <line x1={meanPos} y1="3" x2={meanPos} y2="27" stroke="#556572" strokeWidth="2" />
  
          {/* User's Score Marker */}
          <Tooltip title={`Your Score: ${grade} out of ${max}`}>
            <rect x={gradePos - 7} y="8" width="14" height="14" stroke="#224488" strokeWidth="2" rx="3" fill="#aabbdd" />
          </Tooltip>
        </svg>
      </Box>
    );
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell sx={{ fontWeight: "bold" }}> Your Grade</TableCell>
                    <TableCell sx={{ fontWeight: "bold" }}> Mean Grade</TableCell>
                    <TableCell sx={{ fontWeight: "bold" }}> Max Grade</TableCell>
                    <TableCell sx={{ fontWeight: "bold" }}> Min Grade</TableCell>
                    <TableCell></TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
            ...
            <Table aria-label="collapsible table">
              ...
              <TableBody>
                {grades.map((grade, idx) => {
                  return <Row sx={{ fontWeight: "bold" }} key={`header-${idx}`} row={grade} />;
                })}
              </TableBody>
            </Table>
          </TableContainer>
